...
- **`qbittorrent`** (`github: mjblack/qbittorrent.cr`, 0.2.0) — download-client client, used to list a torrent's files by hash and fetch a torrent snapshot (state, seeds, `added_on`).
...
**Validation** (fail fast at startup): backend `name`/`type`/`url`/`api_key` present; `type` ∈ {sonarr, radarr}; `interval` matches `<int>[m|h|d]` if given; `cleanup.*.timeout` matches the same pattern if given; `extensions_filter` non-empty; ≥1 download client; each download client has a `name` and **either `api_key` or (`username` AND `password`)**.
...
- `config.cr` — `Config`/`Backend`/`DownloadClient`/`Cleanup`/`CleanupRule` (`YAML::Serializable` + `JSON::Serializable`), loading, interval parsing, extension matcher, cleanup defaults (metadata 15m / stalled 60m), validation.
...
- `download_client/` — resolve + connect to qBittorrent (via the `qbittorrent` shard), `files_for(hash)` and `info_for(hash)` (`TorrentSnapshot`: state, seeds, `added_on`).
...
- `janitor.cr` — per-item pipeline: snapshot → metadata-downloading → files/ext → stalled; each kind delete+blocklist → (if released) search; emits via the `Reporter`.
- `store.cr` — SQLite `processed_downloads` (audit) and `download_states` (live first-seen **metadata** and **stalled** clocks).
